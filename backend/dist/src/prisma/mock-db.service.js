...
let MockDbService = MockDbService_1 = class MockDbService {
    ...
    triggerNotification(userId, title, body) {
    ...
    loadFromDisk() {
        ...
        try {
            if (fs.existsSync(filePath)) {
                ...
                this.orders = (parsed.orders || []).map((o) => ({ ...o, createdAt: new Date(o.createdAt) }));
                this.inventoryItems = parsed.inventoryItems || [];
                this.staffShifts = (parsed.staffShifts || []).map((s) => ({ ...s, startTime: new Date(s.startTime), endTime: new Date(s.endTime) }));
